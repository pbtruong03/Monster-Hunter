...
abstract public class Enemyv2 : MonoBehaviour
{
    ...
    protected Vector2 directionAtk; // Vector huong tan cong
    ...
    void FixedUpdate()
    {
        if(attackScript != null && attackScript.detectedObjs != null && attackScript.detectedObjs.Count > 0)
        {
            directionAtk = (attackScript.detectedObjs[0].transform.position - transform.position).normalized;
            Attack();
        }
